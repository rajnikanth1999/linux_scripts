...
EOF
echo "Now you are loged as regular user"
kubectl get nodes
status=$?
if test $status -eq 0
then
    echo "kubernetes is working fine"
    echo "execute 'kubectl get nodes' to check the installation"
else
    mkdir -p $HOME/.kube
    sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
    sudo chown $(id -u):$(id -g) $HOME/.kube/config
    echo "The kubernetes is working fine"
    echo "execute `kubectl get nodes` to check the installation"
fi